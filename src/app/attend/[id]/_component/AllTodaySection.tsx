 ...
 */
function AllTodaySection({ today, all, todayAttendNum, attend, nickname, isCoupon = false, isMe = false }: Props) {
  return (
    <article className="w-[386px] bg-white rounded-md py-6 px-5 shadow-lg flex flex-col gap-6">
      ...
      <p className="text-center mb-3">
        <span className="font-700">{nickname}</span> 님은 오늘 {TODAY_RESULT[attend]}
      </p>
      <div className="flex gap-6">
        <Button disabled={!isMe || !isCoupon} theme="secondary" roundSize="sm" heightSize="sm" customStyle="w-full">
          오늘은 Skip!
        </Button>
        <Button disabled={!isMe} heightSize="sm" roundSize="sm" customStyle="w-full">
          Check-In
        </Button>
      </div>
    </article>
