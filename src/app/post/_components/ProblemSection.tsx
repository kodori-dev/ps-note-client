...
function ProblemSection({ type }: Props) {
  ...
  const { data, isLoading, isSuccess, isOpen, setIsOpen } = useDebouncingSearch(
    boj_id,
    boj_id && boj_id.split('-').length < 2
  );
  ...
  return (
    <PostSectionLayout
      title="Problem"
      description="오늘은 어떤 문제를 풀었나요?"
    >
      <div className="flex flex-col gap-5 w-[373px]">
        <Input
          label="제출 날짜*"
          description="실제로 해당 문제를 제출한 날짜를 선택해 주세요."
          type="date"
          register={register('submitted_at')}
        />
        ...
        <div className="flex flex-col gap-2 text-14 font-700">
          ...
          <div className="flex gap-3">
            <Tag
              customStyle="w-[68px]"
              initialState={is_correct_answer === 'AC'}
              onClickFunc={() => handleTagClick('AC')}
            >
              AC
            </Tag>
            <Tag
              customStyle="w-[68px]"
              initialState={is_correct_answer === 'WA'}
              onClickFunc={() => handleTagClick('WA')}
            >
              WA
            </Tag>
          </div>
          <input
            hidden
            {...register('is_correct_answer', { required: REQUIRED_INPUT })}
          />
        </div>
